...
func run(ctx context.Context, cancel context.CancelFunc, httpPort int, dataDir string) int {
	...
	accessLogsFile, err := os.OpenFile("linko.access.log", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0644)
	if err != nil {
		standardLogger.Printf("failed to created file, error: %v", err)
		return 1
	}
	accessLogger := log.New(accessLogsFile, "INFO: ", log.LstdFlags)
